...
// -------------------------------------------------------------
// HONEST CLIENT-SIDE DETECTION ENGINE (mirrors server/ml/*.py)
// -------------------------------------------------------------
// Post-review fix: the static-host fallbacks below used to hand back the
// seeded ANSWER KEY directly (dataset.rings as "detected" rings, and a
// case's own predicted_atm_location_id as the "prediction") with a fixed
// fake confidence. That's what the public GitHub Pages demo actually runs,
// so it's been replaced with the same non-leaky logic as the Python engine:
// rings are derived from shared mule accounts, and location predictions use
// only OTHER cases' historical outcomes for the same mule hub, never the
// case's own label.

function deriveL2MuleClient(caseObj) {
  const l1 = caseObj.suspicious_account;
  for (const t of memoryTransactions) {
    if (t.case_id !== caseObj.complaint_id) continue;
    if (t.source_account === l1 && String(t.destination_account).includes('MULE')) {
...
function detectRingsClient() {
  const l2ToCases = {};
  memoryComplaints.forEach((c) => {
    const l2 = deriveL2MuleClient(c);
    if (!l2ToCases[l2]) l2ToCases[l2] = [];
    l2ToCases[l2].push(c);
  });

  const detected = [];
  let idx = 1;
  Object.entries(l2ToCases).forEach(([l2Mule, cases]) => {
    if (cases.length < 2) return;
    // Sub-split by entry (L1) mule, same as the Python engine, so two
    // distinct syndicates sharing a layering hub aren't silently merged.
    const l1Groups = {};
    cases.forEach((c) => {
      const l1 = c.suspicious_account;
      if (!l1Groups[l1]) l1Groups[l1] = [];
      l1Groups[l1].push(c);
    });
    Object.entries(l1Groups).forEach(([l1Mule, subCases]) => {
      const memberIds = subCases.map((c) => c.complaint_id).sort();
      const totalDiverted = subCases.reduce((s, c) => s + (c.amount || 0), 0);
      detected.push({
        detected_ring_id: `RING-DET-${String(idx).padStart(2, '0')}`,
        shared_account_id: l2Mule,
        entry_mule_id: l1Mule,
        member_case_ids: memberIds,
        member_count: memberIds.length,
        total_diverted_inr: totalDiverted,
        confidence_score: Math.min(0.98, 0.75 + memberIds.length * 0.04),
      });
      idx += 1;
    });
  });
...
function predictLocationClient(caseId, excludeCaseId = caseId) {
  const caseObj = memoryComplaints.find((c) => c.complaint_id === caseId) || memoryComplaints[0];
  const l1Mule = caseObj.suspicious_account;
  const l2Mule = deriveL2MuleClient(caseObj);
  let hour = 18;
  if (caseObj.transaction_datetime) {
    const m = caseObj.transaction_datetime.match(/(\d{2}):\d{2}:\d{2}$/);
    if (m) hour = parseInt(m[1], 10);
  }

  // Historical affinity from OTHER cases sharing the same L2 mule hub only.
  const hitCounts = {};
  memoryComplaints.forEach((c) => {
    if (c.complaint_id === excludeCaseId) return;
    if (deriveL2MuleClient(c) === l2Mule && c.predicted_atm_location_id) {
      hitCounts[c.predicted_atm_location_id] = (hitCounts[c.predicted_atm_location_id] || 0) + 1;
    }
  });
  const totalHits = Object.values(hitCounts).reduce((a, b) => a + b, 0);
  const historicalHubId = totalHits > 0
    ? Object.entries(hitCounts).sort((a, b) => b[1] - a[1])[0][0]
    : null;
  const hubLoc = historicalHubId ? memoryLocations.find((l) => l.location_id === historicalHubId) : null;
  ...
  const candidates = memoryLocations.map((loc) => {
    const hits = hitCounts[loc.location_id] || 0;
    const muleAffinity = totalHits > 0 ? hits / totalHits : 0.10 * (loc.risk_index || 0.5);
    const timeAlignment = Math.max(0.2, 1.0 - Math.abs(hour - 19) * 0.15);
    let distKm = null;
    let prox = 0.5;
    if (hubLoc) {
      distKm = loc.location_id === historicalHubId ? 0 : haversine(hubLoc.lat, hubLoc.lng, loc.lat, loc.lng);
      prox = loc.location_id === historicalHubId ? 1.0 : Math.max(0.1, 1.0 / (1.0 + distKm / 3.0));
    }
    const density = loc.risk_index || 0.5;
    const rawScore = 0.45 * muleAffinity + 0.20 * prox + 0.15 * timeAlignment + 0.20 * density;

    const reasons = [];
    reasons.push(hits > 0
      ? `Mule hub (${l2Mule}) previously cashed out at this location in ${hits} other complaint(s).`
      : `No prior cash-out history for mule hub (${l2Mule}) at this location — first-seen ring or location.`);
    reasons.push([17, 18, 19, 20, 21].includes(hour)
      ? `Peak withdrawal window alignment (18:00-21:30) matches complaint timestamp (${String(hour).padStart(2, '0')}:00).`
      : `Off-peak operational window (${String(hour).padStart(2, '0')}:00 hours).`);
    reasons.push(hubLoc
      ? `Located ~${distKm.toFixed(1)} km from this ring's known cash-out hub in ${loc.jurisdiction}.`
      : `No established ring hub yet; ranked on general hotspot density in ${loc.jurisdiction}.`);
    ...
    return {
      ...
      lng: loc.lng,
      raw_score: rawScore,
      distance_km: distKm !== null ? Number(distKm.toFixed(1)) : null,
      historical_cashouts: loc.historical_cashouts,
      contributing_features: {
        mule_chain_affinity: Number((muleAffinity * 100).toFixed(1)),
        geospatial_proximity: Number((prox * 100).toFixed(1)),
        temporal_window_match: Number((timeAlignment * 100).toFixed(1)),
        historical_density: Number((density * 100).toFixed(1)),
      },
  ...
  candidates.forEach((c, i) => {
    c.confidence_percentage = Number(Math.min(97, c.raw_score * 100).toFixed(1));
    c.rank = i + 1;
  });
  ...
  return {
    ...
    analyzed_mule_chain: { entry_mule_l1: l1Mule, layering_mule_l2: l2Mule },
    top_ranked_candidate: candidates[0],
    all_ranked_candidates: candidates.slice(0, 6),
    model_metadata: {
...
export async function fetchAllRings() {
  ...
  // Honest fallback: derive rings from shared mule accounts (mirrors the
  // Python detect_rings()), instead of handing back the seeded answer key.
  const detected = detectRingsClient();
  return {
    detected_rings: detected,
    total_rings: detected.length,
    methodology: 'Bipartite Case-Mule Co-occurrence Graph Clustering (client-side, derived — not the seeded answer key)'
  };
}
...
export async function fetchCaseLinkage(caseId) {
  ...
  const l2Mule = deriveL2MuleClient(targetCase);
  // Linked purely via shared L2 mule hub — no shortcut through ground_truth_ring.
  const linked = memoryComplaints.filter(c => c.complaint_id !== targetCase.complaint_id && deriveL2MuleClient(c) === l2Mule);

  return {
    target_case: targetCase,
    linked_case_count: linked.length,
    banner_message: `${linked.length} other active complaints share intermediary mule hub (${l2Mule}).`,
...
export async function runPrediction(caseId) {
  ...
  // Honest fallback: run the same leave-one-out logic as the Python engine
  // instead of echoing this case's own predicted_atm_location_id back as
  // the "prediction" with a fixed fake confidence.
  return predictLocationClient(caseId, caseId);
}
